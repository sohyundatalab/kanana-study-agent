...
@media (max-width: 768px) {
    ...
    .premium-card, .soft-card, .result-card, .taste-card, .taste-grid-card {
        border-radius: 18px;
        padding: 14px;
    }
    .section-title {
...
def get_default_concept_answer() -> str:
    return "개념을 입력한 뒤 버튼을 누르면, 카카오 LLM이 두 문장으로 짧고 쉽게 설명해드려요."
...
if subject == "SQL":
    ...
    if st.button("💛 KANANA 설명 받기 💛", use_container_width=True, key="sql_concept_btn"):
        try:
